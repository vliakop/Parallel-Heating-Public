...
int main (int argc, char *argv[]){
  ...
  /*create initial array. Extended with extra row,cols.*/
  ...
  if (my_rank == MASTER) {
    init_heat = (float*)malloc(NXPROB*NYPROB*sizeof(float));
    MPI_Gather(&u[0][0][0],1,subarray,init_heat,blockSize*blockSize,MPI_FLOAT,MASTER,cart);
    for(a=0;a<blockSize;a++){
      for(m=0;m<sqrt(nproc);m++){
        for(i=0;i<NXPROB;i+=blockSize){
          for(j=0;j<blockSize;j++)
            printf("%d\t",(int)init_heat[a*NXPROB+m*blockSize+i*NXPROB+j]);
        }
        printf("\n");
      }
    }
  ...
  if (my_rank == MASTER) {
    final_heat = (float*)malloc(NXPROB*NYPROB*sizeof(float));
    MPI_Gather(&u[0][0][0],1,subarray,final_heat,blockSize*blockSize,MPI_FLOAT,MASTER,cart);
    for(a=0;a<blockSize;a++){
      for(m=0;m<sqrt(nproc);m++){
        for(i=0;i<NXPROB;i+=blockSize){
          for(j=0;j<blockSize;j++)
            printf("%.1f\t",(float)final_heat[a*NXPROB+m*blockSize+i*NXPROB+j]);
        }
        printf("\n");
      }
    }
